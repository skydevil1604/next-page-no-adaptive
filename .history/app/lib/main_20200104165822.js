...
// 		'transform': "rotateX(" + (x * 40) + "px) rotateY(" +  (y * 40) + "px)"
// 	});
	
// 	$('.main-content__text').css({
// 		'transform': "perspective(2000px) rotateY(" + (y * (-40)) + "px) rotateX(" + (x * (-40)) + "px) translateX(" + (x * 40) + "px) translateY(" +  (y * 40) + "px)"
// 	});
...
function move(x, y) {
  ...
  // центр карточки
  let xser = el.offset().left + el.width() / 2;
  let yser = el.offset().top + el.height() / 2;
  ...
  let raznX = otnX / el.width() * 100 * 2;
  let raznY = otnY / el.height() * 100 * 2;
